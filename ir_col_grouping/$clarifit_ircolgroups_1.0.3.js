  ...
  this.dispColGrp = function(pColGrps,pOptions){
    ...
    if (vTbl[0].rows.length > 0) {
      for (var i = 0; i < apex.jQuery(vTbl[0].rows[1].cells).length; i++){
        ...
        if (typeof(apex.jQuery('.apexir_WORKSHEET_DATA tr:eq(1) th:eq(' + i + ')').attr('id')) != "undefined")
          vColId = apex.jQuery('.apexir_WORKSHEET_DATA tr:eq(1) th:eq(' + i + ')').attr('id').toUpperCase();
